  ...
  persist(
    (set, get) => ({
      ...
      setThemeState: (newState: ThemeEditorState) => {
        ...
        if (
          equal(oldStateWithoutMode, newStateWithoutMode) &&
      ...
      hasThemeChangedFromCheckpoint: () => {
        const checkpoint = get().themeCheckpoint;
        return !equal(get().themeState, checkpoint);
      },
      hasUnsavedChanges: () => {
        ...
        const stylesChanged = !equal(themeState.styles, presetThemeStyles);
        const hslChanged = !equal(
